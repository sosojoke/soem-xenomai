...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary) 
{
   ...
   {
      rt_mutex_create(&(port->getindex_mutex), NULL);
      rt_mutex_create(&(port->tx_mutex)      , NULL);
      rt_mutex_create(&(port->rx_mutex)      , NULL);
      port->sockhandle        = -1;
      port->lastidx           = 0;
   ...
   *psock = rt_dev_socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ECAT));
   ...
   i = 1; */
   timeout = 1000LL;
   r = rt_dev_ioctl(*psock, RTNET_RTIOC_TIMEOUT, &timeout);
   if(r<0)
   ...
   r = rt_dev_ioctl(*psock, SIOCGIFFLAGS, &ifr);
   if(r<0)
...
int ecx_closenic(ecx_portt *port) 
{
   if (port->sockhandle >= 0) 
      rt_dev_close(port->sockhandle);
   if ((port->redport) && (port->redport->sockhandle >= 0))
      rt_dev_close(port->redport->sockhandle);
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   {   
      ...
      rt_dev_send(port->redport->sockhandle, &(port->txbuf2), port->txbuflength2 , 0);
